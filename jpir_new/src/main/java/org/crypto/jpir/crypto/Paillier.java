...
public class Paillier implements HE {
    ...
    public BigInteger encryptString(String message) {
        ...
        // Convert the string message into a byte array using UTF-8 encoding
        byte[] bytes = message.getBytes(StandardCharsets.UTF_8);

        // Convert the byte array into a BigInteger representation
        BigInteger P = new BigInteger(bytes);

        BigInteger r = BigIntegerUtils.randomInterval(key.getN(), rnd);

        // Encrypt the BigInteger representation of the message
