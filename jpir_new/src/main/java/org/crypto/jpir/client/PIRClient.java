...
public class PIRClient implements PIRClient_Intf {
    ...
    public ArrayList<String> decryptResponseVector(ArrayList<BigInteger> responseVector) {
//        System.out.println(responseVector);
        ArrayList<String> resultVector = new ArrayList<>();
        for (BigInteger response : responseVector) {
            BigInteger decryptedValue = decrypt(response);
            String decryptedString = new String(decryptedValue.toByteArray());
            resultVector.add(decryptedString);

        }
        return resultVector;
    }

    private BigInteger decrypt(BigInteger encryptedValue) {
    ...
    public ArrayList<BigInteger> generateQueryVector(int index) {
        ...
        for (int i = 0; i < queryDim - 1; i++) {
            ...
            BigInteger encryptedValue = homEnc.encrypt(BigInteger.valueOf(nonzero)); // Encrypt the nonzero position directly
            queryValues.add(encryptedValue);
    ...
    public ArrayList<BigInteger> generateQueryVector2(int index) {
		if (homEnc == null)
			return null;
